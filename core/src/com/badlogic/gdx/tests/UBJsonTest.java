...
public class UBJsonTest extends GdxTest {
	static final String fn = "C:\\Users\\zones\\Desktop\\新建文件夹 (10)\\test.ubjson";
	...
	public void create () {
		try {
			...
			UBJsonWriter uw = new UBJsonWriter(Gdx.files.absolute(fn).write(false));
			uw.object();
			...
			ur.oldFormat = false;
			JsonValue v = ur.parse(Gdx.files.absolute(fn));
...
//			Gdx.app.log("UBJsonTest", "result = \n" + v.toString());
	...
	private void performanceTest () throws Exception {
		...
		UBJsonWriter uw = new UBJsonWriter(Gdx.files.absolute(fn).write(false, 8192));
		uw.object();
		uw.set("0floats", new float[] {});
		uw.set("3floats", new float[] {1, 2, 3.456789f});
		uw.set("xfloats", new float[] {Float.MIN_VALUE, Float.MAX_VALUE, Float.NaN, Float.NEGATIVE_INFINITY});
		...
		Gdx.app.log("UBJsonTest", "File size is " + Gdx.files.absolute(fn).length());
		UBJsonReader ur = new UBJsonReader();
		ur.oldFormat = false;
		start = System.currentTimeMillis();
		ur.parse(Gdx.files.absolute(fn));
